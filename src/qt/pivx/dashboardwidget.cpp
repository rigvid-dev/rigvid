    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of RIV earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 RIV");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("RIV")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair RIV, zRIV
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(254, 157, 151));
