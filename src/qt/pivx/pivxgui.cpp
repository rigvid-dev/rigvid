...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Rigvid client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Rigvid"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Rigvid"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
