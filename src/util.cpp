...
const char * const PIVX_CONF_FILENAME = "rigvid.conf";
const char * const PIVX_PID_FILENAME = "rigvid.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Rigvid only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\rigvid
// Windows >= Vista: C:\Users\Username\AppData\Roaming\rigvid
// Mac: ~/Library/Application Support/rigvid
// Unix: ~/.rigvid
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rigvid";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\rigvidParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\rigvidParams
    // Mac: ~/Library/Application Support/rigvidParams
    // Unix: ~/.rigvid-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RigvidParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "RigvidParams";
#else
    // Unix
    return pathRet / ".rigvid-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty rigvid.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Rigvid config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override rigvid.conf
