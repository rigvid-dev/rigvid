...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected RIV and zRIV supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zRIVsupply']:
                assert_equal(node['zRIVsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check RIV and zRIV supply at the beginning
        # ------------------------------------------
        # zRIV supply: 2 coins for each denomination
        ...
        # RIV supply: block rewards minus burned fees for minting
        ...
        # Verify that RIV and zRIV supplies were properly updated after the spends and reorgs
        self.log.info("Check RIV and zRIV supply...")
